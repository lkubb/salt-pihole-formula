...
We will see a quick setup to configure the ``pihole`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``pihole`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``pihole`` formulas under ``/srv/salt/pihole/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/pihole/parameters/dns:domain/
    ln -s dns:domain /srv/salt/pihole/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/pihole/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/pihole/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``pihole`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``pihole`` formulas under ``/srv/salt/pihole/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/pihole/parameters/roles
...
We create a configuration for the role ``pihole/server`` in ``/srv/salt/pihole/parameters/roles/pihole/server.yaml``:
...
We create another configuration for the role ``pihole/client`` in ``/srv/salt/pihole/parameters/roles/pihole/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles pihole/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles pihole/client
...
.. code-block:: console
    ...
    mv /srv/salt/pihole/parameters/dns:domain/  '/srv/salt/pihole/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-pihole-as-json:
      test.nop:
        - name: {{ pihole | json }}
...
Here is an example based on `template-formula/pihole/config/file.sls`_:
    ...
    pihole-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='pihole-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ pihole.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``pihole`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ pihole['winner'] }}
...
.. _template-formula/pihole/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/pihole/config/file.sls
